...
parser.add_argument(
    ...
    type=float,
    default=1.0,
    help='linearity of feature mapping')
parser.add_argument(
    '-est',
    '--estimation_method',
    type=str,
    default='gmm',
    help='estimation method: gmm/z_est')
...
"""
x_dim = 1
x_informative_dim = 1 # only the first two entries matter in the state transition
feature_dim = 1 # [x0, x0**2]
...
saved_batches = list(np.arange(0, n_batches, 10))

# Feature mapping
phi = partial(nonlinear_feature_mapping, scale=args.scale)


n_sims = args.n_sims

if args.approx_degree == 1:
    estimate_psi = identity
    # estimate_cov_sqrt_root_func = get_estimate_cov_sqrt_root_degree_1
else:
    estimate_psi = partial(sparse_feature_mapping, args.approx_degree)
    # estimate_cov_sqrt_root_func = get_estimate_cov_sqrt_root_degree_2
...
def exp(s):
    ...
    Y, X, T, Phi, P = get_observations_sparse_linear_markovian(n_exploration, x_dim, L, A, B, C, alpha, beta, gamma, rct_agents, phi, estimate_psi, noise_dis, noise_std, 0)

    results, return_weights, sum_vars, return_H0 = get_estimates(Y, X, T, estimate_psi, P, thetas, betas, kappas, A, B, C, noise_std, 
                                                             estimate_cov_sqrt_root=estimate_cov_sqrt_root_func,
                                                             fit_oracle=False, current_batch_size=n_exploration, sum_vars=np.zeros(L), verbose=False, estimation_method=args.estimation_method)
    ...
    for b in range(1, n_batches):
        ...
        agents = fit_agent(X, T * estimate_psi(X), Y)
        ...
        # Draw observations
        Y_b, X_b, T_b, Phi_b, P_b = get_observations_sparse_linear_markovian(batch_size, x_dim, L, A, B, C, alpha, beta, gamma, agents, phi, estimate_psi, noise_dis, noise_std, floor)
        ...
        results, return_weights, sum_vars, return_H0 = get_estimates(Y, X, T, estimate_psi, P, thetas, betas, kappas,  A, B, C, noise_std,
                                                                 estimate_cov_sqrt_root=estimate_cov_sqrt_root_func, fit_oracle=False,
                                                                 current_batch_size=batch_size, sum_vars=sum_vars, prev_feasible_weights=return_weights, prev_H0=return_H0, verbose=False, estimation_method=args.estimation_method)
        results['true_thetas'] = np.concatenate([[theta_0], thetas.flatten()])
        results['b'] = b
        results_set = append_results(results_set, results)
    ...
    np.savez(save_path,
             ...
             thetas=results_set['true_thetas'], approx_degree=args.approx_degree, scale=args.scale,

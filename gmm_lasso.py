...
def exp(s):
    ...
    Y, X, T, Phi, P = get_observations_sparse_linear_markovian(n_exploration, x_dim, L, A, B, C, alpha, beta, gamma, rct_agents, phi, psi, noise_dis, noise_std, 0)
    ...
    for b in range(1, n_batches + 1):
        ...
        # Draw observations
        Y_b, X_b, T_b, Phi_b, P_b = get_observations_sparse_linear_markovian(batch_size, x_dim, L, A, B, C, alpha, beta, gamma, agents, phi, psi, noise_dis, noise_std, floor)
        ...
        results, return_weights, sum_vars, return_H0 = get_estimates(Y, X, T, psi, P, thetas, betas, kappas,  A, B, C, noise_std,
                                                                     estimate_cov_sqrt_root=estimate_cov_sqrt_root_func, fit_oracle=True,
                                                                     current_batch_size=batch_size, sum_vars=sum_vars,
                                                                     prev_feasible_weights=return_weights, prev_H0=return_H0, verbose=False,
                                                                    estimation_method=args.estimation_method)
        results['true_thetas'] = np.concatenate([[theta_0], thetas.flatten()])
        results['b'] = b
        results_set = append_results(results_set, results)
